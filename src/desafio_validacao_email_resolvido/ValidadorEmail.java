package desafio_validacao_email_resolvido;
...
public class ValidadorEmail {
	...
	public static boolean validar(String email) {
		...
		if (posicaoArroba == -1)
			return false;
		if (posicaoSufixo <= posicaoArroba)
			return false;
		...
		if (!validarNomeUsuario(nomeUsuario))
			return false;
		if (!validarNomeDominio(nomeDominio))
			return false;
	...
	private static boolean validarNomeUsuario(String nomeUsuario) {
		...
		return validarCaracteresEmParteDoEmail(nomeUsuario, true, true, '_', '-', '.');
	}
	...
	private static boolean validarNomeDominio(String nomeDominio) {
		...
		return validarCaracteresEmParteDoEmail(nomeDominio, false, true, '-', '.');
	}

	private static boolean validarSufixoDominio(String sufixoDominio) {
		if (sufixoDominio.length() != 2 && sufixoDominio.length() != 3)
			return false;
	...
	private static boolean validarCaractere(Character caractere, boolean aceitaMaiusculo, boolean aceitaDigito,
		...
		return (Character.isLetter(caractere) && (aceitaMaiusculo || Character.isLowerCase(caractere)))
				|| (aceitaDigito && Character.isDigit(caractere)) || caracteresExtras.contains(caractere);
	}
